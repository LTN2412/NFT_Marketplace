...
public interface AssetRepository extends ReactiveNeo4jRepository<Asset, String> {
        @Query("MATCH (u:user {id:$userId})\n" +
                        ...
                        "MERGE (u)-[r:HAS_IN_CART {isSelect:true}]->(a)")
        ...
        @Query("MATCH (:user {id:$userId})-[:HAS_IN_CART]->(a:asset)\n" +
                        "RETURN a")
        Flux<Asset> findAllAssetsInCartFrom1User(String userId);

        @Query("MATCH (u:user {id:$userId})-[r:HAS_IN_CART]->(a:asset)\n" +
                        "SET r.isSelect=$isSelect")
        Mono<Void> selectAssetInCart(String userId, String asset, Boolean isSelect);

        @Query("MATCH (u:user {id:$userId})-[:HAS_IN_CART {isSelect:true}]->(a:asset)\n" +
                        "RETURN a")
        Flux<Asset> findAllAssetsIsSelectInCartFrom1User(String userId);
}
